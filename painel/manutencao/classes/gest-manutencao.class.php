...
class Manutencao
{
    ...
    public static function manutencaoDireta($dados){
        ...
        if($rows > 0){ 
                ...
                VALUES(".$id_item.",".$id_mov.",'".$user['id_usuario']."','$texto')");
                $rs->execute();
                $rows = $rs->rowCount();
            ...
            if ($nm_filtro) {
                if ($nm_filtro == 'dt_movimentacao') {
                    if (strpos($filtro, '...') !== false) {
                        ...
                        $rs = $db->prepare("SELECT * FROM manutencao WHERE dt_inicio_manutencao BETWEEN '$start' AND '$end' AND dt_fim_manutencao IS NULL ORDER BY id_manutencao DESC");
                    }
                } else {
                    $rs = $db->prepare("SELECT * FROM manutencao WHERE $nm_filtro = '$filtro' AND dt_fim_manutencao IS NULL ORDER BY id_manutencao DESC");
                }
                $rs->execute();
                $resultado = $rs->fetchAll(PDO::FETCH_ASSOC);
                return ["dados" => $resultado];
                ...
                $rs = $db->prepare("SELECT * FROM manutencao WHERE dt_fim_manutencao IS NULL ORDER BY id_manutencao DESC");
                $rs->execute();
                $resultado = $rs->fetchAll(PDO::FETCH_ASSOC);
                return ["dados" => $resultado];
    ...
    public static function getManutencaoEncerrada( $nm_filtro = null, $filtro = null){
        ...
        if ($nm_filtro) {
            if ($nm_filtro == 'dt_movimentacao') {
                if (strpos($filtro, '...') !== false) {
                    list($start, $end) = explode('...', $filtro);
                    $rs = $db->prepare("SELECT * FROM manutencao WHERE dt_inicio_manutencao BETWEEN '$start' AND '$end' AND dt_fim_manutencao IS NOT NULL ORDER BY id_manutencao DESC");
                }
            } else {
                $rs = $db->prepare("SELECT * FROM manutencao WHERE $nm_filtro = '$filtro' AND dt_fim_manutencao IS NOT NULL ORDER BY id_manutencao DESC");
            }
            $rs->execute();
            $resultado = $rs->fetchAll(PDO::FETCH_ASSOC);
            return ["dados" => $resultado];
        } else {
            $rs = $db->prepare("SELECT * FROM manutencao WHERE dt_fim_manutencao IS NOT NULL ORDER BY id_manutencao DESC");
            $rs->execute();
            $resultado = $rs->fetchAll(PDO::FETCH_ASSOC);
            return ["dados" => $resultado];
        }
    ...
    public static function manutencaoItem($id = null){
        ...
        if($rows > 0){ 
            ...
            if($rows > 0){  
                ...
                $rs = $db->prepare("INSERT INTO manutencao (id_item, id_item_movimentacao, id_autor, obs_in)
                VALUES(".$id_item.",".$id_mov.",'".$user['id_usuario']."','".$_POST['descricao']."')");
                $rs->execute();
                $rows = $rs->rowCount();
